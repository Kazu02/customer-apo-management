...
- 営業担当の通常選択肢: 柳沢悠貴 / 岩本拓也 / 菅原貴博 / 村井亮介 / 大島雅史 / 小椋裕也 / 細川貴弘 / 藤森宣哉 / 江口裕人。
- **営業担当名簿は単一の変更点に集約**: `main.js` と `index.html` の `SALES_STAFF` 配列。メンバー増減はこの配列を編集するだけ（両ファイルで内容を一致させる）。フォームの選択肢は `SALES_STAFF` から動的生成、`SALES_STAFF_NAME_MAP`（正規化）も配列から自動生成。苗字・表記ゆれのみ `SALES_STAFF_ALIASES` に足す。
