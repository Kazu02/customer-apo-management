...
## 2026-08-05: 行削除でID採番が壊れ、顧客IDが3組重複した（同日修正済み）

- Problem: `?action=getCustomers` が199行を返すが、ID `189` / `190` / `191` がそれぞれ2行ある。ペアの2件は姓の異なる**別人**。あわせてID `1` / `3` / `13` が欠番。実顧客数はID distinct で196。
- Cause: `main.js:158` の `var newId = sheet.getLastRow();`。IDを**行数**から採番しており、「既存IDの最大+1」ではない。退任者 `須川一輝` 名義のダミー3件を削除して行数が3減ったため、その後の新規登録が既存IDを再利用した。
- 実害: `getCustomerData`（`main.js:136`）も `updateCustomerById`（`main.js:188`）も**最初に一致した行で停止する**。重複ペアの後発1件はフォームから開けず、更新すると同ID先発行を上書きする。実在顧客3名が現在この状態。
- 下流への影響: `顧客向け公式LINE` の移行は GAS顧客IDを `external_mappings.external_id`（org+source_system+external_id が一意）と `import_rows.source_row_key` に使う。重複があると dry-run で落ちる。**是正するまでフェーズA移行を実行しない。**
- 対処（2026-08-05 実施済み・ユーザー承認のうえ実行）:
  1. `registerCustomer` の採番を `nextCustomerId_()`（既存ID最大+1）へ変更。行削除に影響されない。**version 34 として本番デプロイ済み**。
  2. 重複3組の**後発行**（顧客情報シート 190/191/192行）へ未使用ID 200/201/202 を振り直した。先発行は据え置き。
  3. あわせて `アポ報告` C列「ID-会社名-名前」3行を付け替えた。**旧ID + ' - ' + 会社名 + ' - ' + 名前 の完全一致のみ**を対象にし、推測での付け替えはしていない（不一致0件）。
- 事前点検の結果（振り直し前に実測）: `名寄せ` の「紐づけ顧客ID（編集可）」から 189/190/191 への参照 **0件**。`統合顧客管理` に該当ID行 **0件**（この3名の登録後にメニューの再構築を実行していないため）。よって**振込先6列の取り違えは発生していない**。`アポ報告` は6行が該当し、うち3行が振り直し対象の完全一致だった。
- 実行方法: 使い捨てデプロイ（@32 / @33・キー必須の `action=opsTmp` 分岐）から dry-run → apply を実行し、**作業後に両デプロイを削除（404を確認）、一時コード `ops_tmp.js` と `doGet` の分岐も削除して push**。本番デプロイには一時コードを含めていない。
- 検証（本番URLで実測）: 199行・distinct ID 199・**重複0**・ID範囲 2〜202・欠番 1/3/13・名前空0・担当空0。担当別は 江口100 / 岩本90 / 柳沢8 / 藤森1。`getCustomer` は 189 / 200 / 201 / 202 のいずれも15フィールドで個別に引ける（振り直し前は後発行が引けなかった）。`action=opsTmp` と `action=getSalesStaff` はどちらも `unknown action`。GET は 200 / `application/json` / `Access-Control-Allow-Origin: *` で GitHub Pages から従来どおり読める。ローカルとリモートHEADの一致も確認済み。
  - 補足: `curl -I`（HEADメソッド）はGASが非対応で403を返す。**疎通確認はGETで行うこと**。HEADの403を障害と誤認しない。
- デプロイ時の必須確認: 本プロジェクトは `executeAs: USER_DEPLOYING` / `ANYONE_ANONYMOUS` のため、**オーナー以外の clasp 認証で再デプロイすると本番が全面403になる**（2026-08-03 の事故）。`clasp show-authorized-user` でオーナーアカウントを目視確認してから `clasp push` → `clasp redeploy` する。
- 補足: 欠番1/3/13を埋め直さない。IDは連番である必要がなく、埋めると過去データとの対応が壊れる。

## 2026-08-05: getSalesStaff（営業名簿API）をコードから削除した

- 経緯: ID採番の修正を本番へ出すには新しいversionを作る必要があるが、GASのversionはプロジェクト全体のスナップショットなので、HEADに残っていた `getSalesStaff` も同時に配信されてしまう。
- Decision: 2026-08-03 の「公開範囲を広げるだけの利益が無いため配信しない」という判断を維持するため、**分岐そのものをコードから削除**した。version 34 には含まれない（`action=getSalesStaff` は `unknown action`）。
- Consequence: 「remote HEADには入っているが未配信」という乖離が解消し、HEAD＝本番配信内容が一致した状態に戻った。名簿APIが必要になったら、当時の判断どおり共有シークレット必須の形で作り直す。営業10名の氏名は `main.js` / `index.html` / `PROJECT_CONTEXT.md` の `SALES_STAFF` から従来どおり参照できる。
- 未着手のまま残るもの: `getCustomers` が顧客の氏名・会社名・担当営業を無認証で返す件（`index.html` のパスワードはクライアント側のみ）。これは本件と独立した課題として残す。
